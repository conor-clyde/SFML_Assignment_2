    ...
    for (int row = 0; row < num_xCells; ++row)
    {
        for (int col = 0; col < num_yCells; ++col)
        {
            if (cells[row][col] == 1)
            {
                ...
                if (row == 0 || row == 1 || col==0 || col==1 || row==num_yCells-1 || row==num_yCells-2 || col==num_xCells-1 || col==num_xCells-2)
                    _pixel.setFillColor(sf::Color::Color(156, 214, 226, 255));
                if ((col==0 && row==4) || (col==0 && row==5) || (col==1 && row==4) || (col==1 && row==5) || (col==num_xCells-1 && row==num_yCells-5) || (col== num_xCells-1 && row==num_yCells-6) || (col==num_xCells-2 && row==num_yCells-5) || (col == num_xCells - 2 && row == num_yCells - 6))
